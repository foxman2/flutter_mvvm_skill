...
description: >-
  用于已有 Flutter MVVM 项目中的产品经理 UI 预览和受限展示层改动：调整展示型 UI、文案或样式，在 lib/product_preview/ 新增按正式 MVVM 页面命名和结构实现的隔离页面，优先用 mock API 数据辅助界面评审，并产出待开发审核说明。不用于正式业务逻辑、正式 AppPage 导航、正式功能集成、已确认后端 API/model 或正式页面迁移；按任务使用 flutter-mvvm-feature-dev、flutter-mvvm-api-dev 或 flutter-mvvm-mock-api-dev。
---
...
2. 判断任务类型：
   ...
   - 需要数据：优先用 `$flutter-mvvm-mock-api-dev` 的 mock API 模式实现，并标记待开发审核；只有纯布局占位、没有 service 层价值的小型 UI 状态才保留在同目录 ViewModel 中。
