...
PM skill 可以同时使用 `$flutter-mvvm-mock-api-dev` 并按其规则修改临时 contract、AppContainer/ApiService wiring、mock service 和 mock-only model；这不授权实现已确认的正式 API/model 或真实 Dio 请求。
