...
description: >-
  用于已有 Flutter MVVM 项目中的产品经理 UI 预览和受限展示层改动：调整展示型 UI、文案或样式，在 lib/product_preview/ 新增遵循正式页面组织规则的隔离预览页，用 mock 数据辅助界面评审，并产出待开发审核说明。不用于正式业务逻辑、正式 ViewModel 行为、正式 AppPage 导航、正式功能集成、已确认后端 API/model 或正式页面迁移；按任务使用 flutter-mvvm-feature-dev、flutter-mvvm-api-dev 或 flutter-mvvm-mock-api-dev。
---
...
- 可以为预览页面新增同目录 preview-only ViewModel，用于展示状态、临时交互和 demo 数据绑定。
