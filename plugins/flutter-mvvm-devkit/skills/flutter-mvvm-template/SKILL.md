...
description: >-
  创建新的 Flutter MVVM 模板项目并套用 bundled flutter-mvvm-template assets。用于用户要求生成、初始化或脚手架化一个全新的 Flutter MVVM app 时；生成前需要确认 app 显示名和原生包名。不用于修改已有应用、新增正式页面/UI/导航/ViewModel、PM UI 原型、API/model 开发或 mock API 开发；这些任务分别使用 flutter-mvvm-feature-dev、flutter-mvvm-pm-ui、flutter-mvvm-api-dev、flutter-mvvm-mock-api-dev。
---
...
5. 用户要求改造已有 app、新增页面/UI/API/mock 时，说明这个 skill 只负责创建新模板项目，并引导使用对应的 Flutter MVVM 开发 skill。
