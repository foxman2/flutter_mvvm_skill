...
- Page 泛型使用 `<Feature>ViewModelType`，并显式声明 `required super.viewModelProvider`；provider 必须返回非空 ViewModel。
- 普通页面无论是否包含路由或页面运行参数，都由 `AppPage.generateWidgetBuilder()` 传入 provider 闭包延迟创建 ViewModel。
- ViewModel 使用 Service 或 Repository 时通过构造函数接收；AppPage provider 从 `AppContainer.shared` 取得具体依赖并完成组装。
- 普通页面不要先创建 ViewModel 实例再捕获或传入 Page；需要预配置实例的弹窗、ActionSheet 和 child ViewModel 按实际所有权处理。
