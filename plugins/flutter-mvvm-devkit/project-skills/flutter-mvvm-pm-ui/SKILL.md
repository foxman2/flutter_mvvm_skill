...
description: >-
  在已有 Flutter MVVM 项目中完成用户明确要求的 PM 评审、UI 原型、隔离预览或非发布展示调整，并记录最终 PM 改动、后台接口增量和查看入口。用于不改变正式业务行为的产品展示与开发交接；演示业务数据必须由 domain contract 的 Mock 实现提供；不用于正式业务逻辑、真实 Dio 请求或正式页面迁移。
---
...
- 本 skill 不新增或修改 domain API contract、ApiService wiring、mock service、正式 model、真实 Dio 请求、认证、埋点、推送或持久化逻辑；任务包含演示业务数据时，先由适用的数据层工作流完成这些改动。
